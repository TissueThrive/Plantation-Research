...
const CallusUploadComponent = ({
  ...
  useEffect(() => {
    const fetchCurrentHighestID = async () => {
      ...
      querySnapshot.forEach((doc) => {
        const currentID = parseInt(doc.data().callusID.substring(1));
        ...
        if (predictions.length > 0 && predictions[0].label) {
          ...
          if (label === "Callus") {
            ...
            const { downloadInputURL, downloadProcessURL } =
              await uploadImageToFirebase(imageBlob, selectedFile);
  ...
  const uploadImageToFirebase = async (processImageBlob, inputImageBlob) => {
  ...
  const setNotification = async (callusID) => {
    const currentDate = moment();
    const notifyDate = moment(currentDate).add(5, "weeks");
    const dueDate = moment(currentDate).add(6, "weeks");

    const currentDateFormatted = currentDate.format("YYYY-MM-DD");
    const notifyDateFormatted = notifyDate.format("YYYY-MM-DD");
    const dueDateFormatted = dueDate.format("YYYY-MM-DD");

    const username = userData.firstname + " " + userData.lastname;

    const notificationDocRef = doc(db, "notification", currentDateFormatted);
    ...
    <>
      ...
      >
        ...
        <TextField
          ...
          disabled
          InputProps={{
            endAdornment: <LockIcon sx={{ color: "#B0B0B0" }} />,
          }}
          sx={{
            width: "100%",
            fontFamily: "Poppins",
